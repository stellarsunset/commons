 ...
 *
 * <p>While generally useful, this class is primarily intended to be used to simulate Go-style
 * method returns.
 *
 ...
 * }</pre>
 *
 * <p>In Java, which doesn't have multiple assignment, this can be clunkier. {@link Either} is an
 * imperfect attempt to make this slightly better, specifically when used in conjunction with {@link
 * Issue}:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>The above blends a bit of the functional style most Java devs are used to with some of the
 * upside of the Go-like syntax namely:
 *
 * <ul>
 *   <li>Readability - in that we avoid exceptions (checked or otherwise) and the try-catch
 *       song-and-dance everywhere
 *   <li>Safety - clients can extend {@link Issue} (or write their own type) that's {@code sealed},
 *       and enumerate a collection of known {@link Issue}s, some of which may be handleable before
 *       invoking {@link Either#orThrowRight()}
 * </ul>
 */
public record Either<L, R>(Optional<L> left, Optional<R> right) {
  ...
  /**
   * Consume either the left or right side if present and do something with it, but doesn't require
   * type coalescing. Usually useful for logging the content of an {@link Either} before doing
   * something with it.
   *
   * <pre>{@code
   * Either<T, Exception> result = someMethod();
  ...
  /**
   * Shorthand for {@link #orThrowLeft(Function)} given the left-hand type is a subclass of {@link
   * Exception}, this does require that the left value be a {@link RuntimeException}.
   *
   ...
   * }</pre>
   *
   * <p>There is special handling for {@link Issue}s, which can be thrown without a function to map
   * it to an exception.
   */
  public R orThrowLeft() {
    return orThrowLeft(
        left ->
            left instanceof RuntimeException rt
                ? rt
                : left instanceof Issue i
                    ? i.asException()
                    : new NotAnExceptionTypeException(left));
  }
  ...
  public L orThrowRight() {
    return orThrowRight(
        right ->
            right instanceof RuntimeException rt
                ? rt
                : right instanceof Issue i
                    ? i.asException()
                    : new NotAnExceptionTypeException(right));
  }
  ...
  public static class NotAnExceptionTypeException extends RuntimeException {
    public NotAnExceptionTypeException(Object object) {
      super(
          String.format(
              "Unable to implicitly throw non-runtime exception type: %s", object.getClass()));
    }
  }
}
